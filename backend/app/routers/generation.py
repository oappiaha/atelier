...
Enqueue is a FAN-OUT (PARALLEL-1, 2026-07-31): one wada.generate_colorway task
per requested colorway rather than one sequential task per run, so "generate the
remaining N" costs N/concurrency chains of wall-clock time instead of N. The
response shape is unchanged (same estimate, same `requested` list) — see
_enqueue and app.workers.generate for the trie-claim and run-closure protocol.
"""
...
# long legitimate full-sheet run from a dead worker. Under the PARALLEL-1
# fan-out the key is still per STUDY: every child refreshes it and only the
# child that closes the run clears it, so "any child alive" vetoes the sweep.
...
async def _watchdog_sweep(study, session: AsyncSession) -> bool:
    ...
    if study.status == "generating":
        ...
        if study_stuck:
            ...
            # gen_pending is zeroed with the verdict: the children this run was
            # still waiting on are gone, and a leftover count would make the
            # NEXT run's last child think it wasn't last (PARALLEL-1)
            await session.execute(
                text(
                    "UPDATE studies SET status = 'partial', error = :err, "
                    "gen_pending = 0 WHERE id = :id"
                ),
...
async def _enqueue(study_id, colorway_ids: list[str], session: AsyncSession) -> None:
    """Fan out (PARALLEL-1): ONE child task per colorway, all on the wada queue.

    Colorway chains are independent once they diverge past their shared trie
    prefixes, and every node is a ~39s network-bound fal call — so N colorways
    finish in N/concurrency wall-clock time instead of N × chain. The worker
    coordinates the rest: shared prefix nodes are claimed in gen_nodes so only
    one child pays for them, and studies.gen_pending (set below, in the caller's
    transaction) tells the last child to finish that it must close the run.

    A broker that refuses the FIRST child fails the request (503, nothing ran).
    A broker that dies mid-fan-out leaves the already-queued children to run and
    trims the pending counter to what was actually dispatched, so the run still
    closes on its own; the colorways that never got a task stay 'planned' and
    are simply generated by the next request."""
    from app.workers.generate import generate_colorway as task

    for sent, cw_id in enumerate(colorway_ids):
        try:
            task.delay(str(study_id), str(cw_id))
            ...
            logger.error("broker failed after %d/%d children", sent, len(colorway_ids))
            await session.execute(
                text("UPDATE studies SET gen_pending = :n WHERE id = :id"),
                {"n": sent, "id": str(study_id)},
            )
            await session.commit()
            return
...
async def generate(
    ...
    if todo:
        # gen_pending = the number of child tasks this run owes (PARALLEL-1):
        # written with the status flip so the counter and the 'generating' gate
        # can never disagree. The 409 above guarantees no other run is open, so
        # it is an assignment, not an increment — a stale count left by a lost
        # run self-heals here.
        await session.execute(
            text(
                "UPDATE studies SET status = 'generating', error = NULL, "
                "generation_started_at = now(), gen_pending = :n WHERE id = :id"
            ),
            {"id": str(study.id), "n": len(todo)},
        )
    await session.commit()
    if todo:
        await _enqueue(study.id, [str(r.id) for r in todo], session)
...
async def generate_one(
    ...
    await session.execute(
        text(
            "UPDATE studies SET status = 'generating', error = NULL, "
            "generation_started_at = now(), gen_pending = 1 WHERE id = :id"
        ),
        {"id": str(study.id)},
    )
    await session.commit()
    await _enqueue(study.id, [str(target_id)], session)  # a fan-out of one
