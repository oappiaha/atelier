...
| M7–M8 | Generation + contact sheet; palette lib + export | Not started. **M7 gate #1: 3-step chain-drift test** (only untested pipeline assumption). §9 gaps for M7: study PATCH/DELETE (palette browsing currently mints draft rows), study list route. |
